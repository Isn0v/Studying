...
std::shared_ptr<Expression> Exponent::diff(const std::string &variable) const
{
           ...
           v && v->getName() == variable)
  {
    ...
    std::shared_ptr<Sub> n_minus_one = std::make_shared<Sub>(right, std::make_shared<Val>(1));
    return std::make_shared<Mult>(std::make_shared<Mult>(right, std::make_shared<Exponent>(left, n_minus_one)), left->diff(variable));
  }
  else
  {
...
{
  return std::make_shared<Div>(std::make_shared<Sub>(std::make_shared<Mult>(left->diff(variable), right),
...
{
  return std::make_shared<Add>(std::make_shared<Mult>(left->diff(variable), right),
                               std::make_shared<Mult>(left, right->diff(variable)));
}

std::shared_ptr<Expression> Sub::diff(const std::string &variable) const
{
  return std::make_shared<Sub>(std::make_shared<Sub>(left->diff(variable), right), right->diff(variable));
}
...
std::shared_ptr<Expression> Val::diff(const std::string &variable) const { return std::make_shared<Val>(0); }
